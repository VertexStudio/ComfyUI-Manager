...
def is_allowed_security_level(level):
    if level == "block":
        ...
        return core.get_config()["security_level"].lower() in [
            "weak",
            "normal",
            "normal-",
        ]
...
async def get_risky_level(files, pip_packages):
    ...
    json_data2 = await core.get_data_by_mode(
        ...
        channel_url="https://github.com/ltdrdata/ComfyUI-Manager/raw/main",
    )
...
local_db_custom_node_list = os.path.join(
    core.comfyui_manager_path, "custom-node-list.json"
)
local_db_extension_node_mappings = os.path.join(
    core.comfyui_manager_path, "extension-node-map.json"
)
...
def print_comfyui_version():
    ...
    try:
        ...
        try:
            if (
                core.comfy_ui_commit_datetime.date()
        ...
        if "cm.on_revision_detected_handler" in cm_global.variables:
            ...
            print(
                f"[ComfyUI-Manager] Some features are restricted due to your ComfyUI being outdated."
            )
        # <--
    ...
    if "custom_nodes" in json_obj:
        for i, node in enumerate(json_obj["custom_nodes"]):
            ...
            if url in json_obj_github:
                ...
                json_obj["custom_nodes"][i]["last_update"] = json_obj_github[url][
                    "last_update"
                ]
                json_obj["custom_nodes"][i]["trust"] = (
                    json_obj_github[url]["author_account_age_days"] > 180
                )
            else:
    ...
    if data["save_path"] != "default":
        ...
        if model_type == "checkpoints" or model_type == "checkpoint":
            ...
            if folder_paths.folder_names_and_paths.get("diffusion_models"):
                base_model = folder_paths.folder_names_and_paths["diffusion_models"][0][
                    1
                ]
            else:
                print(f"[ComfyUI-Manager] Your ComfyUI is outdated version.")
                base_model = folder_paths.folder_names_and_paths["unet"][0][
                    0
                ]  # outdated version
        else:
            base_model = os.path.join(folder_paths.models_dir, "etc")
...
def check_custom_nodes_installed(
    json_obj, do_fetch=False, do_update_check=True, do_update=False
):
    if do_fetch:
    ...
    if do_fetch:
        ...
        update_exists = any(
            item["installed"] == "Update" for item in json_obj["custom_nodes"]
        )
        if update_exists:
...
async def fetch_updates(request):
    try:
        json_obj = await core.get_data_by_mode(
            request.rel_url.query["mode"], "custom-node-list.json"
        )

        check_custom_nodes_installed(json_obj, True)

        update_exists = any(
            "custom_nodes" in json_obj
            and "installed" in node
            and node["installed"] == "Update"
            for node in json_obj["custom_nodes"]
        )
...
async def update_all(request):
    ...
    try:
        ...
        updated = [
            item["title"]
            for item in json_obj["custom_nodes"]
            if item["installed"] == "Update"
        ]
        failed = [
            item["title"]
            for item in json_obj["custom_nodes"]
            if item["installed"] == "Fail"
        ]
...
async def fetch_customnode_list(request):
    if (
        "skip_update" in request.rel_url.query
        and request.rel_url.query["skip_update"] == "true"
    ):
        skip_update = True
    else:
        skip_update = False
    ...
    json_obj_github = await core.get_data_by_mode(
        request.rel_url.query["mode"], "github-stats.json", "default"
    )
        ...
        if code is not None and code.startswith("#NOTICE_"):
            try:
                ...
                return notice_version[0] < core.version[0] or (
                    notice_version[0] == core.version[0]
                    and notice_version[1] <= core.version[1]
                )
            except Exception:
                return False
        else:
    ...
    json_obj["custom_nodes"] = [
        record
        for record in json_obj["custom_nodes"]
        if not is_ignored_notice(record.get("author"))
    ]
...
async def fetch_customnode_alternatives(request):
    alter_json = await core.get_data_by_mode(
        request.rel_url.query["mode"], "alter-list.json"
    )
...
async def fetch_alternatives_list(request):
    ...
    custom_node_json = await core.get_data_by_mode(
        request.rel_url.query["mode"], "custom-node-list.json"
    )
    ...
    try:
        ...
        if os.path.exists(path):
            ...
            target_path = os.path.join(
                core.startup_script_path, "restore-snapshot.json"
            )
...
async def get_current_snapshot_api(request):
    try:
        return web.json_response(
            core.get_current_snapshot(), content_type="application/json"
        )
    except:
        return web.Response(status=400)
    ...
    for url in files:
        ...
        try:
            ...
            if url.endswith(".py"):
                ...
                path = (
                    os.path.join(core.js_path, js_path_name)
...
def copy_uninstall(files, js_path_name="."):
    for url in files:
        ...
        base_path = (
            core.custom_nodes_path
            if url.endswith(".py")
            else os.path.join(core.js_path, js_path_name)
        )
...
async def install_custom_node(request):
    ...
    if install_type == "copy":
        if "js_path" in json_data:
            if (
                "." in json_data["js_path"]
                or ":" in json_data["js_path"]
                or json_data["js_path"].startswith("/")
            ):
                print(
                    f"[ComfyUI Manager] An abnormal JS path has been transmitted. This could be the result of a security attack.\n{json_data['js_path']}"
                )
                return web.Response(status=400)
            else:
...
async def update_comfyui(request):
    ...
    try:
        ...
        if res == "fail":
            print(
                f"ComfyUI update fail: The installed ComfyUI does not have a Git repository."
            )
            return web.Response(status=400)
        elif res == "updated":
            return web.Response(status=201)
...
async def toggle_active(request):
    ...
    if install_type == "git-clone":
        ...
        res = copy_set_active(
            json_data["files"], not is_disabled, json_data.get("js_path", None)
        )
                ...
                if match:
                    ...
                    try:
                        if (
                            ...
                            markdown_content = (
                                f'<P style="text-align: center; color:red; background-color:white; font-weight:bold">Your ComfyUI is too OUTDATED!!!</P>'
                                + markdown_content
                            )
                    except:
                        pass
        ...
        for json_file in json_files + pack_files:
            ...
            with open(file_path, "r") as file:
                try:
                    ...
                    print(
                        f"[ComfyUI-Manager] Error decoding component file in file {json_file}: {e}"
                    )
...
@PromptServer.instance.routes.post("/model/install_by_url")
async def install_model_by_url(request):
    if not is_allowed_security_level("high"):
        print(SECURITY_MESSAGE_NORMAL_MINUS)
        return web.Response(status=403)
    ...
    try:
        json_data = await request.json()
        model_path = os.path.join(get_model_dir(json_data), json_data["filename"])

        print(f"Install model from URL '{json_data['url']}' into '{model_path}'")
